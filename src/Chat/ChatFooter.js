...
function ChatFooter({ roomName, roomId, db, firebase, storage }) {
  ...
  const [showidKomuPishut, set_RoomId] = useState(false);
  ...
  console.log('КТО Пишет 50000000 ', showidKtoPishet);
...
console.log('Кому Пишут 400000000 ', showidKomuPishut);
        ...
        if (
          ...
          db.collection("rooms")
            ...
            .then(function (docRef) {
              console.log("Док написан с id : ", docRef.id);
              db.collection("rooms")
                .doc(roomId)
                .collection("messages")
                .doc(docRef.id)
          ...
          db.collection("rooms")
            ...
            .add({
              ...
              uid: user.uid,
              timestamp: firebase.firestore.FieldValue.serverTimestamp(),
              idKtoPishet: showidKtoPishet,
              idKomuPishut: showidKomuPishut,
...
<p style={{fontSize:'10px'}}> ChatFooter.js</p>
      <DrawerBottom
      ...
      <p style={{fontSize:'10px',color:"blue"}}> ChatFooter form</p>
      <form>
        {/* <input
          required
          value={input}
